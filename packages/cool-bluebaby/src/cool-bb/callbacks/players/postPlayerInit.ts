import { CacheFlag, CollectibleType, FamiliarVariant, ModCallback } from "isaac-typescript-definitions";
...
function playerInit(player: EntityPlayer) {
    if (player.GetPlayerType() === PlayerTypeCustom.COOL_BB) {
        ...
        const luckyPoopItem = Isaac.GetItemConfig().GetCollectible(CollectibleTypeCustom.LUCKY_POOP);
        const dirtyMindItem = Isaac.GetItemConfig().GetCollectible(CollectibleType.DIRTY_MIND);

        player.AddCollectible(CollectibleTypeCustom.LUCKY_POOP, luckyPoopItem?.MaxCharges ?? 2);
